...
object AndroidQDBUtils : IDBUtils {
  ...
  private fun getUri(idLong: Long, mediaType: Int, isOrigin: Boolean = false): Uri {
  ...
  private var refreshNoInfoLock = ReentrantLock()

  override fun refreshIncompleteInfoAssets(context: Context): Boolean {
    if (refreshNoInfoLock.isLocked) {
      return false
    }

    data class Info(val id: Long, val mediaType: Int)

    refreshNoInfoLock.withLock {
      val cr = context.contentResolver

      val findKeys = arrayOf(
              MediaStore.MediaColumns._ID, MEDIA_TYPE
      )

      val selection = "( $MEDIA_TYPE = ? AND (${MediaStore.MediaColumns.WIDTH} is NULL OR ${MediaStore.MediaColumns.WIDTH} = 0) )" +
              "OR" +
              "( $MEDIA_TYPE = ? AND (${MediaStore.MediaColumns.DURATION} is NULL OR ${MediaStore.MediaColumns.DURATION} = 0 OR ${MediaStore.MediaColumns.WIDTH} is NULL OR ${MediaStore.MediaColumns.WIDTH} = 0 ) )"

      val args = arrayOf(MEDIA_TYPE_IMAGE.toString(), MEDIA_TYPE_VIDEO.toString())

      val cursor = cr.query(allUri, findKeys, selection, args, null) ?: return true
      ...
      for (info in infos) {
        if (info.mediaType == MEDIA_TYPE_IMAGE) {
          refreshImageInfo(context, info.id)
        } else if (info.mediaType == MEDIA_TYPE_VIDEO) {
          refreshVideoInfo(context, info.id)
        }
      }
  ...
  private fun refreshImageInfo(context: Context, id: Long) {
    val uri = getUri(id, MEDIA_TYPE_IMAGE)
    val cr = context.contentResolver
    ...
    inputStream?.use {
      ...
      val result = cr.update(allUri, cv, idSelection, arrayOf(id.toString()))
      println("update $uri result = $result, cv: $cv")
    }
  }

  private fun refreshVideoInfo(context: Context, id: Long) {
    val uri = getUri(id, MEDIA_TYPE_VIDEO)
    val cr = context.contentResolver
    val pfd = cr.openFileDescriptor(uri, "r") ?: return
    val fd = pfd.fileDescriptor ?: return
    val info = VideoUtils.getPropertiesUseMediaPlayer(fd)
